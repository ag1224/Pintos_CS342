...
/* My Implementation */
static bool higher_priority (const struct list_elem *lhs, const struct list_elem *rhs, void *aux UNUSED);
...
bool sleep_ticks_less (const struct list_elem *lhs, const struct list_elem *rhs, void *aux UNUSED);
...
bool sleep_ticks_less (const struct list_elem *a_, const struct list_elem *b_, void *aux UNUSED)
{
  ASSERT (a_ != NULL);
  ASSERT (b_ != NULL);
  const struct alarm *a = list_entry (a_, struct thread, elem);
  const struct alarm *b = list_entry (b_, struct thread, elem);

  return a->ticks < b->ticks;
}

static bool
higher_priority (const struct list_elem *lhs, const struct list_elem *rhs, void *aux UNUSED)
{
  struct thread *a, *b;
...
sort_thread_list (struct list *l)
{
  ...
  list_sort (l, higher_priority, NULL);
}

/* same as higher_priority but use > instead of >= */
static bool
...
enum intr_level thread_priority_temporarily_up (void)
{
  ...
  // cur->base_priority = cur->priority;
